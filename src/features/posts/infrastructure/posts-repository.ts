...
export class PostsRepository {
    ...
    async myLikeStatus (commentId: string, userId: string): Promise<string|null> {
        if (Types.ObjectId.isValid(commentId)) {            
            const post = await this.PostModel.findOne({_id: new Types.ObjectId(commentId)})
            if (!post) return null
    ...
    async setLikeStatus (commentId: string, userId: string, login: string, likeStatus: string): Promise<boolean> {
        if (Types.ObjectId.isValid(commentId)) {
            ...
            const resultPull = await this.PostModel.updateOne({_id: commentId}, {$pull: filter(oldLikeStatus?.likeStatus || 'None')})
            const resultPush = await this.PostModel.updateOne({_id: commentId}, {$push: filter(likeStatus)})
    ...
    async deletePost (id: string): Promise<boolean> {
        if (Types.ObjectId.isValid(id)) {
            const result = await this.PostModel.deleteOne({_id: new Types.ObjectId(id)})
