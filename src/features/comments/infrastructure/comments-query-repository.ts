...
import { CommentPaginatorType, CommentsFilter } from "../api/dto/middle/comments-middle-dto";
import { CommentDocument, CommentModelType, CommentForPost } from "../domain/comments-entity";
...
export class CommentsQueryRepository {
    ...
    async findComments (postId: string | null = null, filter: CommentsFilter = commentFilter, userId: string = ''): Promise<CommentPaginatorType> {
        ...
        const dbResult = await this.CommentModel.find(find).sort({[filter.sortBy]: (filter.sortDirection == 'asc' ? 1 : -1)}).skip(skip).limit(filter.pageSize);
...
const commentMapper = (comment: CommentDocument, userId: string): CommentOutput => {
    let myStatus: string
