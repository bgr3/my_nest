...
import { CommentForPost, CommentDocument, CommentModelType } from "../domain/comments-entity";

@Injectable()
export class CommentsRepository {
    constructor(@InjectModel(CommentForPost.name) private CommentModel: CommentModelType){}
    ...
    async getCommentById (id: string): Promise<CommentDocument | null>{
        if (Types.ObjectId.isValid(id)) {
            const comment = await this.CommentModel.findOne({_id: new Types.ObjectId(id)});
            return comment
        }
    ...
    async myLikeStatus (commentId: string, userId: string): Promise<string|null> {
        if (Types.ObjectId.isValid(commentId)) {            
            const comment = await this.CommentModel.findOne({_id: new Types.ObjectId(commentId)}).lean()
    ...
    async setLikeStatus (commentId: string, userId: string, oldStatus: string, newStatus: string): Promise<boolean> {
        ...
        if (Types.ObjectId.isValid(commentId)) {
            const resultPull = await this.CommentModel.updateOne({_id: commentId}, {$pull: oldStatusFilter})
            const resultPush = await this.CommentModel.updateOne({_id: commentId}, {$push: newStatusFilter})
    ...
    async deleteComment (id: string): Promise<boolean> {
        if (Types.ObjectId.isValid(id)) {
            const result = await this.CommentModel.deleteOne({_id: new Types.ObjectId(id)})
